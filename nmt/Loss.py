...
class NMTLossCompute(nn.Module):
    ...
    def __init__(self, generator, tgt_vocab):
        ...
        self.padding_idx = tgt_vocab.stoi[tgt_vocab.PAD]
    ...
    def compute_loss(self, batch, weight, output, target):
        scores = self.generator(self.bottle(output))
        if weight is not None:
            scores = self.unbottle(scores, batch.batch_size)
            scores = self.bottle((scores * weight.view(1, -1, 1)))
    ...
    def sharded_compute_loss(self, batch, output, shard_size, weight = None):
        """
        ...
        for shard in shards(shard_state, shard_size):
            loss, stats = self.compute_loss(batch, weight, **shard)
    ...
    def monolithic_compute_loss(self, batch, output):
        ...
        shard_state = self.make_shard_state(batch, output)
        _, batch_stats = self.compute_loss(batch, None, **shard_state)
